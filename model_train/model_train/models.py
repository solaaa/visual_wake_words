...
def resnet_identity(input_batch, model_setting, dropout_prob_ph):
    ...
    input_channel = [8,16,32]

    stage = [2,1,1] # each stage includes N block
    ...
    #avg_pool =  tf.keras.layers.AveragePooling2D(pool_size=(8, 8))(di)
    #flatten = tf.keras.layers.Flatten()(avg_pool)
    # 4. fc
    #fc1 = fc_layer(flatten, input_channel[-1]*4*4, 64, mode=init_mode, name = 'fc1')
    #fc1 = activation(fc1, activation_mode)
    logits = fc_layer(avg_pool, 32, 2, mode=init_mode, name = 'output_fc')
